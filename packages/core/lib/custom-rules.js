    ...
    this.patterns.forEach((compiledPattern) => {
      if (compiledPattern.type === "regex") {
        ...
        const pattern = new RegExp(
          compiledPattern.pattern.source,
        ...
        const searchCode = compiledPattern.caseSensitive
          ? code
          : code.toLowerCase();
  ...
  validateRule(config) {
    ...
    Object.entries(RULE_SCHEMA).forEach(([field, schema]) => {
      ...
      if (
        schema.enum &&
        config[field] &&
        !schema.enum.includes(config[field])
      ) {
        errors.push(
          `Invalid value for ${field}: ${config[field]}. Must be one of: ${schema.enum.join(", ")}`,
        );
      }
    });
