 ...
 */
const ENHANCED_INFRA_FILE_REGEX =
  ...
  traverse(ast, {
        ...
        if (secretType) {
          findings.push(
            createFinding(
              ...
              "HARDCODED_SECRET",
              "CRITICAL",
              `${secretType} detected - move to environment variable`,
              10,
              code,
              { cwe: "CWE-798" },
            ),
          ...
          if (
            ...
            findings.push(
              createFinding(
                ...
                "HARDCODED_SECRET",
                "HIGH",
                "Potential hardcoded secret - move to environment variable",
                9,
                code,
                { cwe: "CWE-798" },
              ),
      ...
      if (callee.type === "Identifier" && callee.name === "Function") {
        findings.push(
          createFinding(
            ...
            "CRITICAL",
            "new Function() creates code from strings - code execution risk",
            10,
            code,
            { cwe: "CWE-95" },
          ),
        ...
        if (arg && arg.type !== "StringLiteral") {
          findings.push(
            createFinding(
              ...
              "REGEX_INJECTION",
              "MEDIUM",
              "Dynamic RegExp - escape user input to prevent ReDoS",
              5,
              code,
              { cwe: "CWE-1333" },
            ),
    ...
    CallExpression(path) {
          ...
          if (firstArg?.type === "StringLiteral") {
            findings.push(
              createFinding(
                ...
                "IMPLICIT_EVAL",
                "HIGH",
                `${name} with string argument executes code like eval()`,
                7,
                code,
                { cwe: "CWE-95" },
              ),
          ...
          if (firstArg && firstArg.type !== "StringLiteral") {
            findings.push(
              createFinding(
                ...
                "COMMAND_INJECTION",
                "CRITICAL",
                `${name}() with dynamic input - command injection risk`,
                10,
                code,
                { cwe: "CWE-78" },
              ),
            ...
            findings.push(
              createFinding(
                ...
                "COMMAND_EXEC",
                "HIGH",
                `${name}() executes system commands - validate inputs`,
                8,
                code,
                { cwe: "CWE-78" },
              ),
          ...
          if (
            ...
            findings.push(
              createFinding(
                ...
                "SSRF_RISK",
                "MEDIUM",
                "Dynamic URL in fetch - validate to prevent SSRF",
                6,
                code,
                { cwe: "CWE-918" },
              ),
      ...
      if (callee.type === "MemberExpression") {
        ...
        if (obj === "vm" || imports.get(obj) === "vm") {
          if (
            ["runInContext", "runInNewContext", "runInThisContext"].includes(
              method,
            )
          ) {
            findings.push(
              createFinding(
                ...
                "VM_CODE_EXEC",
                "CRITICAL",
                `vm.${method} executes arbitrary code`,
                10,
                code,
                { cwe: "CWE-94" },
              ),
        ...
        if (DANGEROUS_PATTERNS.COMMAND_METHODS.includes(method)) {
          ...
          if (firstArg && firstArg.type !== "StringLiteral") {
            findings.push(
              createFinding(
                ...
                "COMMAND_INJECTION",
                "CRITICAL",
                `${method}() with dynamic input - command injection`,
                10,
                code,
                { cwe: "CWE-78" },
              ),
          ...
          if (options) {
            const shellProp = options.properties?.find(
              (p) => p.key?.name === "shell" && p.value?.value === true,
            );
            if (shellProp) {
              findings.push(
                createFinding(
                  ...
                  "SHELL_INJECTION",
                  "CRITICAL",
                  "spawn with shell:true enables shell injection",
                  10,
                  code,
                  { cwe: "CWE-78" },
                ),
          ...
          if (DANGEROUS_PATTERNS.WEAK_ALGORITHMS.includes(algo)) {
            findings.push(
              createFinding(
                ...
                "WEAK_CRYPTO",
                "MEDIUM",
                `${algo.toUpperCase()} is cryptographically weak - use SHA-256+`,
                6,
                code,
                { cwe: "CWE-328" },
              ),
        ...
        if (DANGEROUS_PATTERNS.SYNC_FS.includes(method)) {
          findings.push(
            createFinding(
              ...
              "SYNC_IO",
              "MEDIUM",
              `${method}() blocks event loop - use async version`,
              5,
              code,
            ),
          );
          ...
          if (
            ...
            findings.push(
              createFinding(
                ...
                "PATH_TRAVERSAL",
                "HIGH",
                "File operation with dynamic path - validate to prevent traversal",
                8,
                code,
                { cwe: "CWE-22" },
              ),
          ...
          if (
            ...
            findings.push(
              createFinding(
                ...
                "SQL_INJECTION",
                "CRITICAL",
                "SQL with template literal - use parameterized queries",
                10,
                code,
                { cwe: "CWE-89" },
              ),
            ...
            findings.push(
              createFinding(
                ...
                "SQL_INJECTION",
                "CRITICAL",
                "SQL with string concatenation - use parameterized queries",
                10,
                code,
                { cwe: "CWE-89" },
              ),
        ...
        if (
          ...
          if (query?.type === "ObjectExpression") {
            const hasWhere = query.properties?.some(
            ...
            if (hasWhere) {
              findings.push(
                createFinding(
                  ...
                  "NOSQL_INJECTION",
                  "CRITICAL",
                  "$where enables code execution - avoid or sanitize",
                  9,
                  code,
                  { cwe: "CWE-943" },
                ),
        ...
        if (obj === "Object" && method === "assign") {
          ...
          const hasDynamicSource = sources.some(
            (arg) =>
          ...
          if (hasDynamicSource) {
            findings.push(
              createFinding(
                ...
                "MEDIUM",
                "Object.assign with dynamic source - filter __proto__",
                5,
                code,
                { cwe: "CWE-1321" },
              ),
          ...
          if (htmlArg && htmlArg.type !== "StringLiteral") {
            findings.push(
              createFinding(
                ...
                "XSS",
                "HIGH",
                "insertAdjacentHTML with dynamic content - sanitize",
                8,
                code,
                { cwe: "CWE-79" },
              ),
          ...
          if (urlArg && urlArg.type !== "StringLiteral") {
            findings.push(
              createFinding(
                ...
                "OPEN_REDIRECT",
                "MEDIUM",
                "window.open with dynamic URL - validate origin",
                5,
                code,
                { cwe: "CWE-601" },
              ),
          ...
          if (urlArg && urlArg.type !== "StringLiteral") {
            findings.push(
              createFinding(
                ...
                "OPEN_REDIRECT",
                "MEDIUM",
                "Server redirect with dynamic URL - validate",
                6,
                code,
                { cwe: "CWE-601" },
              ),
          ...
          if (!options || options.type !== "ObjectExpression") {
            findings.push(
              createFinding(
                ...
                "INSECURE_COOKIE",
                "MEDIUM",
                "Cookie set without security flags - add secure, httpOnly, sameSite",
                6,
                code,
                { cwe: "CWE-614" },
              ),
        ...
        if (
          ...
          findings.push(
            createFinding(
              ...
              "CRITICAL",
              "yaml.load is unsafe - use yaml.safeLoad or yaml.load with schema",
              9,
              code,
              { cwe: "CWE-502" },
            ),
    ...
    AssignmentExpression(path) {
      ...
      if (left.type === "MemberExpression") {
          ...
          if (right.type !== "StringLiteral") {
            findings.push(
              createFinding(
                ...
                "XSS",
                "HIGH",
                `${prop} with dynamic content - sanitize to prevent XSS`,
                8,
                code,
                { cwe: "CWE-79" },
              ),
        ...
        if (DANGEROUS_PATTERNS.PROTO_KEYS.includes(prop)) {
          findings.push(
            createFinding(
              ...
              "PROTOTYPE_POLLUTION",
              "HIGH",
              `${prop} assignment enables prototype pollution`,
              8,
              code,
              { cwe: "CWE-1321" },
            ),
        ...
        // Open redirect
        if (
          (objName === "window" || objName === "location") &&
          ...
          if (right.type !== "StringLiteral") {
            findings.push(
              createFinding(
                ...
                "OPEN_REDIRECT",
                "MEDIUM",
                "Dynamic location assignment - validate URL",
                6,
                code,
                { cwe: "CWE-601" },
              ),
          ...
          if (right?.value === "0" || right?.value === 0) {
            findings.push(
              createFinding(
                ...
                "TLS_DISABLED",
                "HIGH",
                "TLS verification disabled - enables MITM attacks",
                8,
                code,
                { cwe: "CWE-295" },
              ),
      ...
      if (keyName === "__proto__") {
        findings.push(
          createFinding(
            ...
            "PROTOTYPE_POLLUTION",
            "HIGH",
            "__proto__ in object literal enables prototype pollution",
            8,
            code,
            { cwe: "CWE-1321" },
          ),
      ...
      if (keyName === "$where") {
        findings.push(
          createFinding(
            ...
            "NOSQL_INJECTION",
            "CRITICAL",
            "$where enables code execution in MongoDB",
            9,
            code,
            { cwe: "CWE-943" },
          ),
          ...
          if (path.key === 0 || path.listKey === "arguments") {
            findings.push(
              createFinding(
                ...
                "SSRF_RISK",
                "MEDIUM",
                "Template literal in HTTP request URL - validate to prevent SSRF",
                6,
                code,
                { cwe: "CWE-918" },
              ),
...
async function analyzeFile(filePath, options = {}) {
  ...
  const result = analyzeEnhanced(
    code,
    filePath,
    options.projectRoot || path.dirname(filePath),
  );
