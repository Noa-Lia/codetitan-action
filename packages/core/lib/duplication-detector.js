...
function extractBlocks(code, minLines = 5) {
  ...
  for (let i = 0; i <= lines.length - windowSize; i++) {
    ...
    const nonEmptyLines = blockLines.filter(
      (l) =>
    ...
    for (const [hash, matches] of blockHashes) {
      ...
      for (let i = 0; i < matches.length; i++) {
        for (let j = i + 1; j < matches.length; j++) {
          ...
          if (m1.file === m2.file) {
            const overlap = !(
              m1.block.endLine < m2.block.startLine ||
  ...
  generateFindings(results) {
    ...
    if (results.totalClones > 10) {
      findings.push({
        ...
        suggestion:
          "Consider refactoring duplicate code into shared functions or modules",
      });
    }
    ...
    for (const clone of results.clones.slice(0, 10)) {
      ...
      findings.push({
        ruleId: isInternal
          ? "duplication/internal-clone"
          : "duplication/cross-file-clone",
