...
class MemoryItem {
  ...
  getDecay() {
    ...
    const decay =
      Math.exp(-ageHours / (24 * 7)) * this.importance + accessBoost;
...
class AgentMemory {
  ...
  async rememberFixDecision(fix, wasAccepted, context = {}) {
    ...
    return this.remember({
      type: wasAccepted
        ? MEMORY_TYPES.SUCCESSFUL_FIX
        : MEMORY_TYPES.REJECTED_FIX,
  ...
  async recallFixDecisions(finding, context = {}) {
    ...
    const accepted = memories.filter(
      (m) => m.memory.type === MEMORY_TYPES.SUCCESSFUL_FIX,
    );
    const rejected = memories.filter(
      (m) => m.memory.type === MEMORY_TYPES.REJECTED_FIX,
    );
  ...
  async learnFromHistory(decisions) {
    for (const decision of decisions) {
      await this.rememberFixDecision(decision.fix, decision.wasAccepted, {
  ...
  async enforceLimit() {
    ...
    for (const { memory } of toRemove) {
      ...
      if (this.supabase) {
        await this.supabase.from("agent_memories").delete().eq("id", memory.id);
      }
    }
  }
  ...
  getStats() {
    ...
    return {
      ...
      byImportance,
      averageAccess:
        this.memories.size > 0 ? totalAccess / this.memories.size : 0,
