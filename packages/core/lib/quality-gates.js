...
class QualityGateResult {
  ...
  toCliOutput() {
    const icon =
      this.status === "PASSED" ? "✅" : this.status === "WARNING" ? "⚠️" : "❌";
    const lines = [
    ...
    this.conditions.forEach((c) => {
      ...
      lines.push(
        `   ${status} ${c.conditionName}: ${c.actual} (threshold: ${comparison} ${c.threshold})`,
      );
    });
...
function evaluateCondition(conditionName, config, metrics) {
  ...
  switch (conditionName) {
    ...
    case "duplicatedLines":
      ...
      metricAvailable =
        metrics.duplicatedLinesPercent !== null &&
        metrics.duplicatedLinesPercent !== undefined;
      break;
    case "codeCoverage":
      ...
      metricAvailable =
        metrics.codeCoverage !== null && metrics.codeCoverage !== undefined;
      break;
...
function evaluateQualityGate(issues, options = {}) {
  ...
  // Get the profile
  const gateProfile =
    customConditions || GATE_PROFILES[profile] || GATE_PROFILES.RECOMMENDED;
