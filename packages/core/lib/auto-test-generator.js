...
class FunctionExtractor {
      ...
      traverse(ast, {
        ...
        ArrowFunctionExpression: (path) => {
          if (path.parent.type === "VariableDeclarator") {
            ...
            if (name) {
              this.functions.push(
                this.extractFunctionInfo(path.node, code, name),
              );
            }
          }
        },
        ClassMethod: (path) => {
          ...
          if (methodName && !["constructor"].includes(methodName)) {
            this.functions.push(
              this.extractMethodInfo(path.node, code, className, methodName),
            );
          }
        },
      });
  ...
  detectSideEffects(body) {
    ...
    if (/\.(save|insert|update|delete|remove)\(/i.test(body))
  ...
  generateEdgeCases(funcInfo, param) {
    ...
    for (const { value, description } of edgeValues) {
      const args = funcInfo.params
        .map((p) => (p.name === param.name ? value : this.getDefaultValue(p)))
        .join(", ");
...
${this.config.expect}(results).toHaveLength(3);`,
  ...
  formatTestFile(funcInfo, testCases, options = {}) {
    ...
    const modulePath =
      options.modulePath || `./${options.fileName?.replace(/\.[jt]sx?$/, "")}`;
    ...
    for (const testCase of testCases) {
      lines.push(
        `  ${this.config.it}('${testCase.name}', ${funcInfo.isAsync ? "async " : ""}() => {`,
      );
...
class AutoTestGenerator {
  ...
  estimateCoverage(tests) {
    ...
    const coverage = Math.min(
      100,
      ((totalTests * avgLinesPerTest) / estimatedLines) * 100,
    );
