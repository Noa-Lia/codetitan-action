...
class CIOutcomeRecorder {
  ...
  async recordTestResults(testResults) {
    ...
    if (scoresError) {
      this.log(
        `⚠️  Could not fetch confidence scores: ${scoresError.message}`,
        "error",
      );
    ...
    for (const score of scores) {
      const hasOutcome = outcomes.find(
        (o) => o.confidence_score_id === score.id,
      );
      if (!hasOutcome) {
  ...
  async recordDeploymentResults(deploymentResults) {
    ...
    for (const score of scores) {
      outcomes.push({
        ...
        outcome_status:
          success && allSmokeTestsPassed && allHealthChecksPassed
            ? "SUCCESS"
            : "FAILURE",
  ...
  findMatchingTest(score, tests) {
    ...
    const fileMatches = tests.filter(
      (t) =>
        t.file &&
        score.file_path &&
        t.file.includes(path.basename(score.file_path)),
    );
  ...
  static getCIInfo() {
    ...
    const info = {
      ...
      prNumber:
        process.env.GITHUB_PR_NUMBER || process.env.CI_MERGE_REQUEST_IID,
    };
