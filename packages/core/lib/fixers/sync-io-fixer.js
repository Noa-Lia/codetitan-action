...
class SyncIOFixer {
  constructor() {
    ...
    this.patterns = [
      {
        from: /fs\.readFileSync\(/g,
        to: "await fs.promises.readFile(",
        name: "readFile",
      },
      {
        from: /fs\.writeFileSync\(/g,
        to: "await fs.promises.writeFile(",
        name: "writeFile",
      },
      {
        from: /fs\.appendFileSync\(/g,
        to: "await fs.promises.appendFile(",
        name: "appendFile",
      },
      {
        from: /fs\.readdirSync\(/g,
        to: "await fs.promises.readdir(",
        name: "readdir",
      },
      {
        from: /fs\.mkdirSync\(/g,
        to: "await fs.promises.mkdir(",
        name: "mkdir",
      },
      {
        from: /fs\.rmdirSync\(/g,
        to: "await fs.promises.rmdir(",
        name: "rmdir",
      },
      { from: /fs\.rmSync\(/g, to: "await fs.promises.rm(", name: "rm" },
      {
        from: /fs\.unlinkSync\(/g,
        to: "await fs.promises.unlink(",
        name: "unlink",
      },
      { from: /fs\.statSync\(/g, to: "await fs.promises.stat(", name: "stat" },
      {
        from: /fs\.lstatSync\(/g,
        to: "await fs.promises.lstat(",
        name: "lstat",
      },
      {
        from: /fs\.renameSync\(/g,
        to: "await fs.promises.rename(",
        name: "rename",
      },
      {
        from: /fs\.copyFileSync\(/g,
        to: "await fs.promises.copyFile(",
        name: "copyFile",
      },
      {
        from: /fs\.chmodSync\(/g,
        to: "await fs.promises.chmod(",
        name: "chmod",
      },
      {
        from: /fs\.chownSync\(/g,
        to: "await fs.promises.chown(",
        name: "chown",
      },
      { from: /fs\.linkSync\(/g, to: "await fs.promises.link(", name: "link" },
      {
        from: /fs\.symlinkSync\(/g,
        to: "await fs.promises.symlink(",
        name: "symlink",
      },
      {
        from: /fs\.readlinkSync\(/g,
        to: "await fs.promises.readlink(",
        name: "readlink",
      },
      {
        from: /fs\.realpathSync\(/g,
        to: "await fs.promises.realpath(",
        name: "realpath",
      },
      {
        from: /fs\.truncateSync\(/g,
        to: "await fs.promises.truncate(",
        name: "truncate",
      },
      {
        from: /fs\.utimesSync\(/g,
        to: "await fs.promises.utimes(",
        name: "utimes",
      },
    ];
  }
  ...
  async fix(finding, config) {
    try {
      ...
      const enhancedCode = this.enhanceWithAsyncAndTryCatch(
        modifiedCode,
        lineIndex,
      );
  ...
  enhanceWithAsyncAndTryCatch(code, changedLine) {
    ...
    if (!functionLine.includes("async ")) {
      ...
      if (functionLine.match(/^\s*function\s+\w+\s*\(/)) {
        // function name()
        lines[functionStart] = functionLine.replace(
          /(\s*)function(\s+\w+\s*\()/,
          "$1async function$2",
        );
        ...
        functionLine.match(/^\s*\w+\s*[:=]\s*(async\s+)?\([^)]*\)\s*=>/)
      ) {
        // Arrow function: name = () =>
        if (!functionLine.includes("async ")) {
          lines[functionStart] = functionLine.replace(
        ...
        functionLine.match(/^\s*const\s+\w+\s*=\s*(async\s+)?\([^)]*\)\s*=>/)
      ) {
        // const name = () =>
        if (!functionLine.includes("async ")) {
          lines[functionStart] = functionLine.replace(
