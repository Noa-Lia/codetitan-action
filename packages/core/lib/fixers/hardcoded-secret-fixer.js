  ...
  async fix(finding, config) {
    try {
      ...
      const patterns = [
        ...
        {
          regex:
            /(const|let|var)\s+(\w*(?:password|secret|key|token|api[_-]?key)\w*)\s*=\s*['"]([^'"]+)['"]/i,
        ...
        {
          regex:
            /(\w*(?:password|secret|key|token|api[_-]?key)\w*)\s*:\s*['"]([^'"]+)['"]/i,
      ...
      if (!matched) {
        ...
        fixedLine =
          originalLine +
          " // TODO: Move hardcoded credential to environment variable";
      }
