...
class CommandExecFixer {
    ...
    try {
      ...
      if (!transformation) {
        if (originalLine.includes("exec(") && originalLine.includes("${")) {
          fixedLine =
            originalLine +
            " /* CRITICAL: Command injection vulnerability - use execFile */";
          transformation = "Added critical security warning";
          confidence = 0.9;
        } else if (originalLine.includes("exec(")) {
          fixedLine =
            originalLine + " // TODO: Consider execFile for safer execution";
          transformation = "Added safety recommendation";
          confidence = 0.65;
        } else if (
          originalLine.includes("spawn(") &&
          !originalLine.includes("[")
        ) {
          fixedLine =
            originalLine +
            " // TODO: Use array arguments to prevent shell injection";
  ...
  ensureExecFileImport(code, fixedLine) {
    ...
    const hasExecFileImport = lines.some(
      (line) =>
