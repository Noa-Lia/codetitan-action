...
function formatCheckRun(analysisResult, options = {}) {
  ...
  const conclusion = qualityGate?.passed
...
async function postPRAnnotations(findings, options) {
  ...
  if (!owner || !repo || !prNumber || !commitSha) {
    throw new Error(
      "postPRAnnotations: owner, repo, prNumber, and commitSha are required",
    );
  }
...
async function createFixPR(fixes, options) {
  ...
  if (!owner || !repo)
    throw new Error("createFixPR: owner and repo are required");
  if (!fixes || fixes.length === 0)
    throw new Error("createFixPR: no fixes provided");
  ...
  if (!baseRef.object || !baseRef.object.sha) {
    throw new Error(
      `createFixPR: could not resolve SHA for branch '${baseBranch}': ${JSON.stringify(baseRef)}`,
    );
  }
