...
class AgentSpawner extends EventEmitter {
  constructor(messageBus, registry, options = {}) {
    ...
    console.log(
      "[AgentSpawner] Initialized with max concurrent agents:",
  ...
  loadAgentTemplates() {
    ...
    if (this.registry && this.registry.registry.agents) {
      this.registry.registry.agents.forEach((agent) => {
        ...
        if (agentFile.includes("..") || path.isAbsolute(agentFile)) {
          console.warn(
            `[WARNING] Skipped potentially unsafe agent file path: ${agentFile}`,
          );
          return;
        }
    ...
    console.log(
      `[AgentSpawner] Loaded ${this.agentTemplates.size} agent templates`,
    );
  }
  ...
  async spawn(agentName, config = {}) {
    ...
    if (this.agents.size >= this.options.maxConcurrentAgents) {
      throw new Error(
        `Maximum concurrent agents reached (${this.options.maxConcurrentAgents})`,
      );
    }
    ...
    if (this.registry) {
      const resolve =
        typeof this.registry.resolveAgent === "function"
  ...
  createAgentInstance(template, config) {
    ...
    const instance = {
      ...
      runtimePolicy: resolveRoleProfile(template, {
        action: "analyze",
        metadata: { role: template.role },
      }),
      performance: {
    ...
    try {
      ...
      if (message.type === "request" && this.messageBus) {
        if (typeof this.messageBus.respondToDelegatedTask === "function") {
          await this.messageBus.respondToDelegatedTask(
            message,
            instance.id,
            response,
          );
      ...
      instance.performance.averageResponseTime =
        instance.performance.totalResponseTime /
        instance.performance.requestsProcessed;
      ...
      console.error(
        `[AgentSpawner] Agent ${instance.id} error:`,
        error.message,
      );
      ...
      // Send error response
      if (message.type === "request" && this.messageBus) {
        const failureResponse = this.createDelegationFailureResponse(
          instance,
          message,
          error,
        );
  ...
  async processAgentTask(instance, message) {
    const delegatedTaskRequest =
      message?.content?.delegation?.taskRequest || null;
    const taskId =
      delegatedTaskRequest?.taskId ||
      message?.content?.task_id ||
      message?.message_id ||
      `${instance.id}-${Date.now()}`;
    const action =
      delegatedTaskRequest?.action ||
    ...
    try {
      ...
      if (instance.sdk) {
        ...
        const quality =
          typeof executionResult?.quality === "number"
  ...
  async attemptRecovery(instance) {
    if (instance.state.recoveryAttempts >= this.options.maxRecoveryAttempts) {
      console.error(
        `[AgentSpawner] Agent ${instance.id} exceeded max recovery attempts, terminating`,
      );
    ...
    console.log(
      `[AgentSpawner] Attempting recovery for ${instance.id} (attempt ${instance.state.recoveryAttempts})`,
    );
  ...
  async terminate(instanceId, reason = "manual") {
    ...
    // Calculate lifetime
    const lifetime =
      new Date(instance.terminatedAt) - new Date(instance.createdAt);
    ...
    this.metrics.averageLifetime =
      this.metrics.totalUptime / this.metrics.agentsTerminated;

    // Emit event
    this.emit("agent:terminated", { instance, reason });

    console.log(
      `[AgentSpawner] Terminated agent: ${instanceId} (reason: ${reason}, lifetime: ${(lifetime / 1000).toFixed(1)}s)`,
    );
  }
  ...
  getInstancesByName(agentName) {
    return Array.from(this.agents.values()).filter(
      (instance) => instance.name === agentName,
    );
  }
  ...
  async healthCheck(instance) {
    try {
      ...
      if (idleTime > this.options.agentTimeout && instance.status === "busy") {
        throw new Error(
          `Agent timeout (idle for ${(idleTime / 1000).toFixed(1)}s)`,
        );
      }
      ...
      if (instance.resources.cpu > this.options.resourceLimits.maxCpu) {
        console.warn(
          `[AgentSpawner] Agent ${instance.id} high CPU usage: ${instance.resources.cpu}%`,
        );
      }

      if (instance.resources.memory > this.options.resourceLimits.maxMemory) {
        console.warn(
          `[AgentSpawner] Agent ${instance.id} high memory usage: ${(instance.resources.memory / 1024 / 1024).toFixed(1)}MB`,
        );
      }

      // Check error rate
      const errorRate =
        instance.state.tasksFailed / Math.max(instance.state.tasksCompleted, 1);
      ...
      console.error(
        `[AgentSpawner] Health check failed for ${instance.id}:`,
        error.message,
      );
  ...
  getMetrics() {
    return {
      ...
      utilizationPercent:
        (this.agents.size / this.options.maxConcurrentAgents) * 100,
      agentsByStatus: {
  ...
  printMetrics() {
    ...
    console.log(
      `| Active Agents: ${metrics.activeAgents}/${metrics.maxConcurrentAgents} (${metrics.utilizationPercent.toFixed(1)}%)${" ".repeat(30 - (metrics.activeAgents + "/" + metrics.maxConcurrentAgents).length)} |`,
    );
    console.log(
      `| +- Active: ${metrics.agentsByStatus.active.toString().padEnd(51)} |`,
    );
    console.log(
      `| +- Idle: ${metrics.agentsByStatus.idle.toString().padEnd(53)} |`,
    );
    console.log(
      `| +- Busy: ${metrics.agentsByStatus.busy.toString().padEnd(53)} |`,
    );
    console.log(
      "|                                                            |",
    );
    ...
    console.log(
      `| +- Agents Spawned: ${metrics.agentsSpawned.toString().padEnd(42)} |`,
    );
    console.log(
      `| +- Agents Terminated: ${metrics.agentsTerminated.toString().padEnd(39)} |`,
    );
    console.log(
      `| +- Agents Failed: ${metrics.agentsFailed.toString().padEnd(45)} |`,
    );
    console.log(
      `| +- Agents Recovered: ${metrics.agentsRecovered.toString().padEnd(41)} |`,
    );
    console.log(
      "|                                                            |",
    );
    console.log(
      `| Average Agent Lifetime: ${(metrics.averageLifetime / 1000).toFixed(1)}s${" ".repeat(30 - (metrics.averageLifetime / 1000).toFixed(1).length)} |`,
    );
    console.log(
      "+------------------------------------------------------------+\n",
    );
  ...
  listAgents() {
    ...
    instances.forEach((instance) => {
      const uptime = (
        (Date.now() - new Date(instance.createdAt)) /
        1000
      ).toFixed(0);
      const taskStatus = `${instance.state.tasksSucceeded}/${instance.state.tasksCompleted} tasks`;
      console.log(
        `| ${instance.id.substring(0, 35).padEnd(35)} | ${instance.status.padEnd(6)} | ${uptime}s |`,
      );
    });
