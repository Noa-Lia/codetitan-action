...
const CONTAINER_RULES = {
  ...
  USER: {
    ...
    no_user: {
      ...
      severity: "MEDIUM",
      cwe: "CWE-250",
      check: (content) => !content.match(/^USER\s+/im),
    ...
    sudo_installed: {
      ...
      cwe: "CWE-250",
      pattern:
        /apt-get\s+install.*\bsudo\b|apk\s+add.*\bsudo\b|yum\s+install.*\bsudo\b/i,
      message:
        "sudo installed in container - unnecessary privilege escalation vector",
    },
  },
  ...
  IMAGE: {
    ...
    untrusted_base: {
      ...
      cwe: "CWE-829",
      pattern:
        /^FROM\s+(?!(?:alpine|ubuntu|debian|node|python|golang|rust|nginx|redis|postgres|mysql|mongo|mcr\.microsoft|gcr\.io|ghcr\.io|docker\.io\/library))/im,
      message:
        "Using potentially untrusted base image - prefer official images",
    },
    privileged_base: {
      id: "container/privileged-base",
      severity: "HIGH",
      cwe: "CWE-250",
      pattern: /^FROM.*\/(root|admin|privileged)/im,
  ...
  SECRETS: {
    env_password: {
      ...
      cwe: "CWE-798",
      pattern:
        /^ENV\s+\w*(PASSWORD|SECRET|KEY|TOKEN|CREDENTIAL|API_KEY)\w*\s*=\s*\S+/im,
    ...
    arg_secret: {
      ...
      severity: "HIGH",
      cwe: "CWE-798",
      pattern: /^ARG\s+\w*(PASSWORD|SECRET|KEY|TOKEN)\w*\s*=/im,
    ...
    copy_private_key: {
      ...
      severity: "CRITICAL",
      cwe: "CWE-798",
      pattern: /^(COPY|ADD)\s+.*\.(pem|key|p12|pfx|jks)/im,
  ...
  NETWORK: {
    ...
    expose_many: {
      ...
      severity: "LOW",
      cwe: "CWE-284",
      check: (content) => (content.match(/^EXPOSE\s+/gim) || []).length > 5,
    ...
    privileged_port: {
      ...
      cwe: "CWE-284",
      pattern:
        /^EXPOSE\s+([1-9]|[1-9][0-9]|[1-9][0-9]{2}|10[0-1][0-9]|102[0-3])\b/im,
  ...
  FILES: {
    ...
    add_tar: {
      ...
      severity: "MEDIUM",
      cwe: "CWE-829",
      pattern: /^ADD\s+\S+\.(tar|tar\.gz|tgz|tar\.bz2)\s/im,
  ...
  BUILD: {
    multiple_cmd: {
      ...
      severity: "LOW",
      cwe: "",
      check: (content) => (content.match(/^CMD\s+/gim) || []).length > 1,
    ...
    multiple_entrypoint: {
      ...
      severity: "LOW",
      cwe: "",
      check: (content) => (content.match(/^ENTRYPOINT\s+/gim) || []).length > 1,
    ...
    no_healthcheck: {
      ...
      severity: "LOW",
      cwe: "",
      check: (content) => !content.match(/^HEALTHCHECK\s+/im),
  ...
  HARDENING: {
    ...
    apk_no_cache: {
      ...
      cwe: "",
      check: (content) =>
        content.includes("apk add") && !content.includes("--no-cache"),
