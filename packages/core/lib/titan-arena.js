...
class TitanArena extends EventEmitter {
  ...
  async compete(finding, fileContent, options = {}) {
    ...
    this._log(
      `Arena: competing for ${finding.category || finding.id} in ${finding.file_path || "unknown"}`,
    );
    ...
    try {
      ...
      for (const fix of fixes) {
        const contender = await this._evaluateContender(
          fix,
          finding,
          fileContent,
          options,
        ...
        this._log(
          `  [${fix.provider}] score=${contender.score.toFixed(3)} tests=${contender.evaluation?.testsPass}`,
        );
      }
      ...
      if (best.score >= this.config.minPassScore) {
        ...
        this._log(
          `  Winner: ${winner.provider} (score=${winner.score.toFixed(3)})`,
        );
        ...
        if (!dryRun && winner.score >= this.config.autoApplyThreshold) {
          ...
          this._log(
            `  Score ${winner.score.toFixed(3)} < autoApplyThreshold ${this.config.autoApplyThreshold} — suggest only`,
          );
        }
      } else {
        this._log(
          `  All contenders below minPassScore (${this.config.minPassScore}) — rejecting all`,
        );
  ...
  async _generateFixes(finding, fileContent, options) {
    ...
    const results = await Promise.allSettled(
      selected.map((provider) =>
        this._generateFixForProvider(finding, fileContent, provider, options),
      ),
    );
  ...
  async _generateFixForProvider(finding, fileContent, provider, options) {
    try {
      ...
      const generator = new FixGenerator(aiManager, {
        preferredProvider: provider,
      });

      const result = await generator.generateFix(finding, fileContent, {
        provider,
      });
  ...
  async createWorktree(name) {
    ...
    if (handle.mode === "directory_copy" && handle.fallbackReason) {
      this._log(
        `  Worktree creation failed (${handle.fallbackReason}), falling back to directory copy`,
      );
    }
