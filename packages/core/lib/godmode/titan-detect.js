...
const PERFORMANCE_RULES = [
  ...
  },
  {
    ...
    category: "performance",
    pattern:
      /JSON\.parse\s*\(\s*(?:fs\.readFileSync|await\s+fs\.promises\.readFile)/,
...
class TitanDetect {
      ...
      lines.forEach((line, idx) => {
        if (rule.pattern.test(line)) {
          ...
          this.stats.byCategory[rule.category] =
            (this.stats.byCategory[rule.category] || 0) + 1;
          this.stats.bySeverity[rule.severity] =
            (this.stats.bySeverity[rule.severity] || 0) + 1;
        }
      });
    }
  ...
  filterBySeverity(issues) {
    ...
    return issues.filter(
      (issue) => (severityOrder[issue.severity] || 5) <= threshold,
    );
  }
