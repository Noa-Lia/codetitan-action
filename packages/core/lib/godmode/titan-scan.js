...
class Level1BasicAnalysis {
  ...
  async analyze(projectPath) {
    ...
    for (const file of files) {
      try {
        ...
        results.metrics.languageBreakdown[ext] =
          (results.metrics.languageBreakdown[ext] || 0) + 1;
    ...
    if (results.files.length > 0) {
      const totalComplexity = results.files.reduce(
        (sum, f) => sum + f.metrics.complexity,
        0,
      );
      results.metrics.averageComplexity =
  ...
  discoverFiles(dirPath, files = []) {
    try {
      ...
      for (const entry of entries) {
        ...
        if (
          this.config.ignorePatterns.some(
  ...
  async analyzeFile(filePath) {
    ...
    if (this.config.parseAST && [".js", ".jsx", ".ts", ".tsx"].includes(ext)) {
      try {
        ...
        result.metrics = {
          ...result.metrics,
          ...this.extractMetricsFromAST(ast),
        };
  ...
  detectIssues(ast, content, filePath) {
    ...
    lines.forEach((line, idx) => {
      ...
      if (
        magicMatch &&
        ![3000, 3001, 8080, 443, 80, 1000, 1024].includes(
          parseInt(magicMatch[1]),
        )
      ) {
        issues.push({
          type: "magic-number",
