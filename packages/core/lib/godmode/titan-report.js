...
class Level3Reporting {
  ...
  generateMarkdownReport(results) {
    ...
    lines.push(
      `| Total Lines | ${results.metrics.totalLines.toLocaleString()} |`,
    );
    ...
    if (results.summary.criticalCount > 0) {
      lines.push("\n## 🔴 Critical Issues\n");
      this.addIssueSection(
        lines,
        results.issues.filter((i) => i.severity === "CRITICAL"),
      );
    }
    ...
    if (results.summary.highCount > 0) {
      ...
      this.addIssueSection(
        lines,
        results.issues.filter((i) => i.severity === "HIGH"),
      );
    }
    ...
    if (results.summary.mediumCount > 0) {
      ...
      this.addIssueSection(
        lines,
        results.issues.filter((i) => i.severity === "MEDIUM").slice(0, 20),
      );
    }
  ...
  addIssueSection(lines, issues) {
    issues.slice(0, 20).forEach((issue) => {
      ...
      lines.push(
        `- **File**: \`${issue.relativePath || issue.file}\`:${issue.line}`,
      );
...
<body>
  ...
  generateInsights(results) {
    ...
    if (
      ...
      !issues.some((i) => i.file.includes("test"))
    ) {
      recommendations.push({
        priority: "MEDIUM",
        title: "Add Test Coverage",
        description:
          "No test files detected. Consider adding unit tests for critical paths.",
        effort: "high",
      });
    }
