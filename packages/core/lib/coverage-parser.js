...
async function parseCobertura(xmlPath) {
  ...
  for (const packageMatch of packageMatches) {
    ...
    const classMatches = packageContent.matchAll(
      /<class[^>]*filename="([^"]+)"[^>]*>([\s\S]*?)<\/class>/g,
    );
    ...
    for (const classMatch of classMatches) {
      ...
      // Parse line coverage
      const lineMatches = classContent.matchAll(
        /<line\s+number="(\d+)"[^>]*hits="(\d+)"[^>]*\/>/g,
      );
      ...
      const methodMatches = classContent.matchAll(
        /<method\s+name="([^"]+)"[^>]*line="(\d+)"[^>]*>/g,
      );
