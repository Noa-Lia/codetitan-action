...
function getDebtRating(ratio) {
  ...
  if (ratio <= 0.1)
    return { rating: "B", label: "Acceptable", color: "lightgreen" };
  if (ratio <= 0.2) return { rating: "C", label: "Moderate", color: "yellow" };
  if (ratio <= 0.5) return { rating: "D", label: "High", color: "orange" };
  return { rating: "E", label: "Critical", color: "red" };
}
