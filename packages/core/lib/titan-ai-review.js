...
const CONFIDENCE_FACTORS = {
  DIRECT_USER_INPUT: {
    weight: 1.0,
    description: "Direct user input (e.g., req.body)",
  },
  INDIRECT_PROPAGATION: {
    weight: 0.8,
    description: "Indirectly tainted via assignment",
  },
  HEURISTIC_NAME: {
    weight: 0.5,
    description: "Variable name suggests user input",
  },
  ...
  HIGH_SIGNAL_SINK: {
    weight: 1.0,
    description: "Sink has high exploit potential",
  },
};
...
function analyzeContext(ast, code, lineNumber, filePath) {
  ...
  Object.entries(SAFE_CONTEXTS).forEach(([key, ctx]) => {
    if (ctx.patterns.some((p) => p.test(filePath))) {
      context[
        `is${key.charAt(0) + key.slice(1).toLowerCase().replace(/_/g, "")}`
      ] = true;
    }
  });
    ...
    enter(path) {
      ...
      if (startLine <= lineNumber && lineNumber <= endLine) {
        ...
        if (path.node.type === "IfStatement") {
          ...
          if (
            /typeof|instanceof|\.length|\.test|\.match|startsWith|includes/.test(
              condCode,
            )
          ) {
    ...
    CallExpression(path) {
      ...
      Object.entries(SANITIZERS).forEach(([type, sanitizer]) => {
        ...
        if (
          ...
          sanitizer.functions?.includes(callee.name)
        ) {
...
function titanAIReview(code, filePath, options = {}) {
  ...
  const enhancedFindings = dataFlowResults.issues.map((finding) => {
    ...
    const exploitDifficulty = calculateExploitDifficulty(
      finding,
      context,
      sanitization,
    );
    if (exploitDifficulty > 0.7) {
      ...
      smartFinding.addReason(
        "Exploit difficulty is high due to multiple mitigations",
        0.4,
      );
  ...
  const falsePositiveRate =
    totalFindings > 0 ? (falsePositives.length / totalFindings) * 100 : 0;
  const avgConfidence =
    enhancedFindings.length > 0
      ? enhancedFindings.reduce((sum, f) => sum + f.confidence, 0) /
  ...
  const scores = {
    ...
    signalToNoise:
      analysisResults.issues.length > 0
        ? Math.min(
            100,
            (analysisResults.metrics.truePositives /
              (analysisResults.metrics.totalRawFindings || 1)) *
              100,
          )
        : 100,
  };
