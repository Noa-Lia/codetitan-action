...
function resolveGitRoot(targetPath) {
  ...
  if (result.status !== 0 || !result.stdout.trim()) {
    throw new Error(
      `changed-only requires a git repository. ${result.stderr.trim() || "git rev-parse failed."}`.trim(),
    );
  }
...
function listChangedFiles(targetPath, options = {}) {
  const gitRoot = options.gitRoot
    ? path.resolve(options.gitRoot)
    : resolveGitRoot(targetPath);
  ...
  if (diffResult.status !== 0) {
    throw new Error(
      `Unable to enumerate changed files. ${diffResult.stderr.trim() || "git diff failed."}`.trim(),
    );
  }
  ...
  const candidates = [
    ...
    ...(untrackedResult.status === 0
      ? parseChangedFileLines(untrackedResult.stdout, gitRoot)
      : []),
  ];
...
function createChangedFilesWorkspace(projectRoot, changedFiles, options = {}) {
  ...
  const workspaceRoot =
    ...
    fs.mkdtempSync(path.join(os.tmpdir(), "codetitan-changed-"));
