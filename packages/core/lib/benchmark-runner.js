...
class BenchmarkRunner {
  ...
  async runOWASPBenchmark(benchmarkPath) {
    if (!fs.existsSync(benchmarkPath)) {
      return {
        error:
          "OWASP Benchmark not found. Download from: https://github.com/OWASP-Benchmark/BenchmarkJava",
      };
    }
    ...
    const expectedResults =
      await this.loadOWASPExpectedResults(expectedResultsPath);
  ...
  generateMarkdownReport(results) {
    ...
    for (const [category, data] of Object.entries(results.byCategory)) {
      ...
      lines.push(
        `| ${category} | ${data.tp} | ${data.fp} | ${data.tn} | ${data.fn} | ${precision}% | ${recall}% |`,
      );
    }
