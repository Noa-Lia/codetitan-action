      ...
      if (callee.type === "Identifier" && callee.name === "Function") {
        findings.push(
          createFinding(
            ...
            "CRITICAL",
            "Function constructor creates code from strings - security risk",
            10,
            code,
          ),
        );
        ...
        if (name === "eval") {
          findings.push(
            createFinding(
              ...
              "EVAL_USAGE",
              "CRITICAL",
              "Dynamic evaluation executes arbitrary code - severe security risk",
              10,
              code,
            ),
          );
        ...
        if (
          ...
          findings.push(
            createFinding(
              ...
              "IMPLICIT_EVAL",
              "HIGH",
              `${name} with string argument executes code dynamically`,
              7,
              code,
            ),
          );
          ...
          if (firstArg && firstArg.type !== "StringLiteral") {
            findings.push(
              createFinding(
                ...
                "COMMAND_INJECTION",
                "CRITICAL",
                `${name}() with dynamic input - potential command injection`,
                10,
                code,
              ),
            );
            ...
            findings.push(
              createFinding(
                ...
                "COMMAND_EXEC",
                "HIGH",
                `${name}() executes system commands - validate inputs carefully`,
                8,
                code,
              ),
            );
          ...
          if (
            ...
            findings.push(
              createFinding(
                ...
                "INSECURE_HTTP",
                "MEDIUM",
                "HTTP request without TLS - use HTTPS for secure data transfer",
                5,
                code,
              ),
            );
          ...
          if (firstArg && firstArg.type !== "StringLiteral") {
            findings.push(
              createFinding(
                ...
                "COMMAND_INJECTION",
                "CRITICAL",
                `${method}() with dynamic input - potential command injection`,
                10,
                code,
              ),
            );
          ...
          if (SECURITY_RULES.WEAK_ALGORITHMS.includes(algo)) {
            findings.push(
              createFinding(
                ...
                "WEAK_CRYPTO",
                "MEDIUM",
                `${algo.toUpperCase()} is cryptographically weak - use SHA-256 or better`,
                6,
                code,
              ),
            );
          ...
          if (
            ...
            findings.push(
              createFinding(
                ...
                "PATH_TRAVERSAL",
                "HIGH",
                "File operation with dynamic path - validate and sanitize to prevent traversal",
                8,
                code,
              ),
            );
          ...
          if (
            ...
            findings.push(
              createFinding(
                ...
                "MEDIUM",
                "Object.assign with dynamic source - validate to prevent prototype pollution",
                5,
                code,
              ),
            );
        ...
        if (prop === "__proto__") {
          findings.push(
            createFinding(
              ...
              "PROTOTYPE_POLLUTION",
              "HIGH",
              "Direct __proto__ assignment enables prototype pollution",
              8,
              code,
            ),
          );
          ...
          if (right.type !== "StringLiteral") {
            findings.push(
              createFinding(
                ...
                "OPEN_REDIRECT",
                "MEDIUM",
                "Location assignment with dynamic value - validate URL to prevent open redirect",
                6,
                code,
              ),
            );
...
async function analyze(targetPath, options = {}) {
  ...
  if (stats.isFile()) {
    ...
    return analyzeWithAST(
      code,
      targetPath,
      options.projectRoot || path.dirname(targetPath),
    );
  }
  ...
  if (stats.isDirectory()) {
    ...
    for (const file of files) {
      ...
      const result = analyzeWithAST(
        code,
        file,
        options.projectRoot || targetPath,
      );
