...
function generateCycloneDX(projectInfo, components) {
  ...
  return {
    ...
    components: components.map((c, i) => ({
      ...
      purl: c.purl,
      scope: c.scope,
      properties: [{ name: "ecosystem", value: c.ecosystem }],
    })),
  };
}
...
function generateSPDX(projectInfo, components) {
  ...
  return {
    ...
    relationships: components.map((c, i) => ({
      ...
      relationshipType:
        c.scope === "required" ? "DEPENDS_ON" : "DEV_DEPENDENCY_OF",
    })),
  };
}
...
class SBOMGenerator {
  ...
  async generate(projectPath) {
    ...
    if (npmData) {
      ...
      allComponents.push(
        ...npmData.components.filter(
          (c) => this.includeDevDeps || c.scope === "required",
        ),
      );
    }
