...
class CustomRule {
  ...
  match(code, filePath) {
    ...
    if (this.compiledPatterns.inside) {
      const contextMatches = this.matchPattern(
        this.compiledPatterns.inside,
        code,
      );
    ...
    if (this.compiledPatterns.not.length > 0) {
      ...
      allMatches = allMatches.filter(
        (m) => !exclusions.has(`${m.line}:${m.column}`),
      );
    }
  ...
  matchPattern(compiled, code) {
    ...
    if (compiled.type === "semgrep") {
      ...
      const searchStr = compiled.caseSensitive
        ? compiled.pattern
        : compiled.pattern.toLowerCase();
...
const BUILTIN_RULES = [
  // SQL Injection
  {
    ...
    message:
      "Potential SQL injection: user input $INPUT concatenated into query",
    cwe: "CWE-89",
    owasp: ["A03:2021"],
  ...
  },
  {
    ...
    pattern: "query(`... ${$VAR} ...`)",
    message:
      "Potential SQL injection: variable $VAR interpolated in query template",
    cwe: "CWE-89",
    owasp: ["A03:2021"],
    confidence: 0.85,
  ...
  },
  {
    ...
    message:
      "Potential XSS: unsanitized $INPUT passed to dangerouslySetInnerHTML",
    cwe: "CWE-79",
    owasp: ["A03:2021"],
    confidence: 0.75,
  ...
  // Error Handling
  {
    ...
    patterns: [{ regex: "catch\\s*\\([^)]*\\)\\s*\\{\\s*\\}", flags: "gm" }],
  ...
  // JWT
  {
    ...
    message:
      "Ensure JWT algorithms are explicitly specified and none is rejected",
    cwe: "CWE-347",
    confidence: 0.7,
  ...
  // Prototype Pollution
  {
    ...
    message:
      "Validate keys to prevent prototype pollution (__proto__, constructor)",
    cwe: "CWE-1321",
    confidence: 0.5,
  ...
  loadBuiltinRules() {
    for (const config of BUILTIN_RULES) {
      try {
        ...
        console.warn(
          `Failed to load builtin rule ${config.id}: ${error.message}`,
        );
      }
    }
  }
  ...
  analyze(code, filePath, options = {}) {
    ...
    if (options.minSeverity) {
      ...
      matchingRules = matchingRules.filter(
        (r) => severityOrder.indexOf(r.severity) >= minIndex,
      );
    }
