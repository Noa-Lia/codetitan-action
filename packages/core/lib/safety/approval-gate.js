...
class ApprovalGate {
  ...
  evaluateFix(fix, context = "cli", userPreferences = {}) {
    ...
    if (ALWAYS_REQUIRE_APPROVAL.includes(fix.category?.toUpperCase())) {
      result.reasons.push(
        `Category '${fix.category}' always requires human approval`,
      );
    ...
    if (confidence < policy.minConfidenceForAuto) {
      result.reasons.push(
        `Confidence ${Math.round(confidence * 100)}% below threshold ${policy.minConfidenceForAuto}%`,
      );
    }
    ...
    if (result.riskLevel > policy.maxRiskLevel) {
      result.reasons.push(
        `Risk level ${result.riskLevel} exceeds policy max ${policy.maxRiskLevel}`,
      );
    }
    ...
    if (this.overrides[fix.file]) {
      ...
      if (override.alwaysApprove) {
        ...
        result.reasons.push(
          `File '${fix.file}' has manual override requiring approval`,
        );
      }
    }
  ...
  calculateRiskLevel(fix) {
    ...
    if (
      ["security", "vulnerability", "injection", "xss"].some((k) =>
        fix.category?.toLowerCase().includes(k),
      )
    ) {
  ...
  evaluateBatch(fixes, context, userPreferences = {}) {
    ...
    const summary = {
      total: fixes.length,
      autoApprovable: results.filter((r) => !r.evaluation.requiresApproval)
        .length,
      requiresApproval: results.filter((r) => r.evaluation.requiresApproval)
        .length,
      byRiskLevel: {
        low: results.filter((r) => r.evaluation.riskLevel === RISK_LEVELS.LOW)
          .length,
        medium: results.filter(
          (r) => r.evaluation.riskLevel === RISK_LEVELS.MEDIUM,
        ).length,
        high: results.filter((r) => r.evaluation.riskLevel === RISK_LEVELS.HIGH)
          .length,
        critical: results.filter(
          (r) => r.evaluation.riskLevel === RISK_LEVELS.CRITICAL,
        ).length,
      },
    };
