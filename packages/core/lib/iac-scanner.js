...
const TERRAFORM_RULES = {
  ...
  ENCRYPTION: {
    s3_no_encryption: {
      ...
      cwe: "CWE-311",
      pattern:
        /resource\s+"aws_s3_bucket"\s+[^{]*\{(?![^}]*server_side_encryption_configuration)/s,
    ...
    rds_no_encryption: {
      ...
      cwe: "CWE-311",
      pattern:
        /resource\s+"aws_db_instance"[^}]*storage_encrypted\s*=\s*false/s,
  ...
  SECRETS: {
    hardcoded_secret: {
      ...
      message:
        "Hardcoded secret in Terraform - use variables or secrets manager",
    },
    aws_access_key: {
  ...
  NETWORK: {
    ...
    rdp_open: {
      ...
      cwe: "CWE-284",
      pattern:
        /from_port\s*=\s*3389[^}]*cidr_blocks\s*=\s*\[\s*"0\.0\.0\.0\/0"/s,
...
const KUBERNETES_RULES = {
  ...
  RESOURCES: {
    no_limits: {
      ...
      cwe: "",
      check: (content) =>
        content.includes("containers:") && !content.includes("limits:"),
    ...
    no_requests: {
      ...
      cwe: "",
      check: (content) =>
        content.includes("containers:") && !content.includes("requests:"),
    ...
    no_cpu_limit: {
      ...
      cwe: "",
      check: (content) =>
        content.includes("limits:") && !content.match(/limits:[^}]*cpu:/s),
    ...
    no_memory_limit: {
      ...
      cwe: "",
      check: (content) =>
        content.includes("limits:") && !content.match(/limits:[^}]*memory:/s),
  ...
  IMAGE: {
    ...
    pull_always: {
      ...
      cwe: "",
      check: (content) =>
        !content.includes("imagePullPolicy: Always") &&
  ...
  RBAC: {
    cluster_admin: {
      ...
      cwe: "CWE-250",
      pattern:
        /kind:\s*ClusterRoleBinding[^-]*roleRef:[^-]*name:\s*cluster-admin/s,
  ...
  NETWORK: {
    ...
    no_network_policy: {
      ...
      check: (content) =>
        ...
        !content.includes("NetworkPolicy"),
  ...
  PRACTICES: {
    no_liveness_probe: {
      ...
      cwe: "",
      check: (content) =>
        content.includes("containers:") && !content.includes("livenessProbe:"),
    ...
    no_readiness_probe: {
      ...
      cwe: "",
      check: (content) =>
        content.includes("containers:") && !content.includes("readinessProbe:"),
    ...
    read_only_root: {
      ...
      check: (content) =>
        ...
        !content.includes("readOnlyRootFilesystem: true"),
...
class IaCScanner {
      ...
      try {
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            if (
              [".git", "node_modules", ".terraform", "vendor"].includes(
                entry.name,
              )
            ) {
              continue;
  ...
  async scanProject(projectPath) {
    ...
    for (const file of files) {
      try {
        ...
        if (fileType === "terraform") {
          ...
          const findings = this.scanWithRules(
            content,
            this.terraformRules,
            file,
            "terraform",
    ...
    return {
      filesScanned:
        scannedFiles.terraform.length + scannedFiles.kubernetes.length,
