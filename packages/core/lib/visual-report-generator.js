...
class MermaidGenerator {
  ...
  generateDataFlowDiagram(flows) {
    ...
    for (const flow of flows) {
      if (flow.type === "source") {
        ...
        lines.push(
          `    C->>S: ${flow.variable} → ${this.escape(flow.sinkType)}`,
        );
        lines.push(
          `    Note over S: ${flow.line ? `Line ${flow.line}` : "Vulnerable"}`,
        );
      }
    }
  ...
  generateDependencyGraph(dependencies) {
    ...
    for (const [file, deps] of Object.entries(dependencies)) {
      ...
      for (const dep of deps) {
        ...
        lines.push(
          `    ${fileId}[${this.escape(path.basename(file))}] --> ${depId}[${this.escape(path.basename(dep))}]`,
        );
      }
    }
  ...
  generateCallGraph(callGraph) {
    ...
    for (const [caller, callees] of Object.entries(callGraph)) {
      ...
      for (const callee of callees) {
        ...
        lines.push(
          `    ${callerId}((${this.escape(caller)})) --> ${calleeId}((${this.escape(callee)}))`,
        );
      }
    }
  ...
  escape(str) {
    if (!str) return "";
    return String(str).replace(/"/g, "'").replace(/[<>]/g, "").substring(0, 50);
  }
...
class HTMLReportGenerator {
  ...
  generate(analysisResult) {
    ...
    const diagrams = this.generateDiagrams(
      findings,
      dataFlows,
      callGraph,
      dependencies,
...
class PDFReportGenerator {
  ...
  async generate(analysisResult, outputPath) {
    ...
    try {
      ...
      console.warn(
        "PDF generation requires puppeteer. Returning HTML instead.",
      );
      return htmlPath;
    }
  }
...
class VisualReportGenerator {
  ...
  async generate(analysisResult, format = "html", outputPath = null) {
    ...
    if (!generator) {
      throw new Error(
        `Unknown format: ${format}. Supported: html, pdf, json, sarif`,
      );
    }
