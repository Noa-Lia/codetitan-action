...
class FixGenerator {
  ...
  async generateFix(finding, fileContent, options = {}) {
    ...
    try {
      ...
      _dbg(
        `[FixGenerator] Generating fix for ${finding.category} using ${provider}`,
      );
      ...
      const fixResponse = await this.requestFixFromAI(
        provider,
        prompt,
        finding,
      );
...
Generate the fix now:`;
  ...
  parseFix(aiResponse, finding) {
    try {
      ...
      const responseText =
        aiResponse.issues?.[0]?.message || aiResponse.rawResponse || "";
  ...
  async verifyFix(fix, originalContent, finding) {
    try {
      ...
      const checks = {
        ...
        lengthReasonable:
          Math.abs(fixedContent.length - originalContent.length) <
          originalContent.length * 0.5,
        noDeletedCode: !this.deletedSignificantCode(
          originalContent,
          fixedContent,
        ),
...
**Please provide a corrected fix that passes all checks.`;
    ...
    try {
      const refinedResponse = await this.requestFixFromAI(
        provider,
        refinementPrompt,
        finding,
      );
      const refinedFix = this.parseFix(refinedResponse, finding);
      const refinedVerification = await this.verifyFix(
        refinedFix,
        fileContent,
        finding,
      );
...
Respond with Jest/Mocha test code.`;
    ...
    try {
      ...
      const testResponse = await this.requestFixFromAI(
        provider,
        testPrompt,
        finding,
      );
      ...
      return {
        generated: true,
        code:
          testResponse.issues?.[0]?.suggestion ||
    ...
    for (let i = 0; i < findings.length; i++) {
      ...
      if (options.delayBetweenFixes) {
        await new Promise((resolve) =>
          setTimeout(resolve, options.delayBetweenFixes),
        );
      }
    }
