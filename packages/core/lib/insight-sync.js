...
class InsightSync {
  constructor(options = {}) {
    this.mode =
      options.mode || process.env.CODETITAN_SYNC_MODE || "sqlite-only";
    this.sqlitePath =
      options.sqlitePath ||
      path.join(process.cwd(), "data", "collective-insight.db");
    this.runtimeInsightPath =
      options.runtimeInsightPath ||
      path.join(path.dirname(this.sqlitePath), "agent-runtime-insights.json");
    ...
    if (this.supabaseUrl && !this._isValidUrl(this.supabaseUrl)) {
      console.warn(
        `[WARNING] Invalid Supabase URL provided: ${this.supabaseUrl}`,
      );
  ...
  async init() {
    ...
    if (this.mode !== "sqlite-only") {
      try {
        ...
        console.warn(
          "[WARNING]  Supabase initialization failed, falling back to SQLite-only mode",
        );
  ...
  async createSupabaseSchema() {
    ...
    if (error) {
      ...
      console.log(
        "   Note: Schema creation requires database admin privileges",
      );
      ...
      if (this.mode === "dual-write" || this.mode === "supabase-only") {
        if (this.supabaseReady) {
          try {
            ...
            console.warn(
              "[WARNING]  Supabase write failed, data preserved in SQLite",
            );
  ...
  async ingestToSupabase(report, metadata) {
    ...
    if (findings.length > 0) {
      ...
      if (findingsError) {
        console.warn(
          `[WARNING]  Some findings failed to insert: ${findingsError.message}`,
        );
      }
    }
    ...
    if (applyFixes && report.fixSummary) {
      ...
      if (fixError) {
        console.warn(
          `[WARNING]  Fix summary failed to insert: ${fixError.message}`,
        );
      }
    }
  ...
  async getSupabaseSummary() {
    ...
    const findingsLogged = data.reduce(
      (sum, run) => sum + (run.total_findings || 0),
      0,
    );
    const avgQuality =
      data.length > 0
        ? data.reduce((sum, run) => sum + (run.quality_score || 0), 0) /
          data.length
        : null;
  ...
  async getDashboard(limit = 5) {
    return {
      ...
      supabase: this.supabaseReady
        ? await this.getSupabaseDashboard(limit)
        : null,
      mode: this.mode,
      syncStats: this.stats,
    };
  ...
  async getSupabaseDashboard(limit = 5) {
    ...
    const qualityTrend = {
      ...
      delta:
        trendData?.length === 2
          ? Number(trendData[0].quality_score) -
            Number(trendData[1].quality_score)
          : null,
    };
  ...
  extractFalsePositiveSignatures(record = {}, metadata = {}) {
    const dismissedActions = Array.isArray(metadata.dismissedActions)
      ? metadata.dismissedActions
      : [];
  ...
  extractRiskClues(record = {}) {
    ...
    return toolTrace
      .filter(
        (entry) =>
          entry && (entry.success === false || entry.riskLevel === "high"),
      )
      .slice(0, 10)
      .map((entry) => ({
        tool: entry.tool,
  ...
  sanitizeRuntimeRecord(record = {}, metadata = {}) {
    ...
    const reviewArtifact =
      ...
      runtimeState.reviewArtifact ||
      null;
    const fixSession =
      record.fix_session ||
      record.fixSession ||
      runtimeState.fixSession ||
      null;
    const promoted =
      record.promoted === true || record.repository_modified === true;
    const acceptedActions = [];
    const dismissedActions = [];

    if (
      promoted ||
      record.status === "applied_in_workspace" ||
      record.status === "validated_in_workspace"
    ) {
    ...
    return {
      ...
      reasoningMode:
        runtimeState.reasoningMode || metadata.reasoningMode || "standard",
      ...
      providerUsage: this.sanitizeProviderUsage(
        runtimeState.providerUsage || {},
      ),
      acceptedActions,
      dismissedActions: [
        ...dismissedActions,
      ...
      falsePositiveSignatures: this.extractFalsePositiveSignatures(
        record,
        metadata,
      ),
  ...
  async ingestAgentRuntime(record = {}, metadata = {}) {
    ...
    const entryLimit = Math.max(
      1,
      Math.min(Number(metadata.limit) || 200, 1000),
    );

    store.entries.unshift(sanitized);
    store.entries = store.entries.slice(0, entryLimit);

    await fs.promises.mkdir(path.dirname(this.runtimeInsightPath), {
      recursive: true,
    });
  ...
  async syncHistoricalData(options = {}) {
    if (!this.supabaseReady) {
      throw new Error(
        "Supabase is not ready. Check credentials and connection.",
      );
    }
    ...
    try {
      ...
      const runs = await this.sqlite.all(
          ...
          try {
            if (!dryRun) {
              ...
              if (findings.length > 0) {
                ...
                if (findingsError) {
                  throw new Error(
                    `Findings for run ${run.id}: ${findingsError.message}`,
                  );
                }
              ...
              if (fixSummaries.length > 0) {
                ...
                if (fixError) {
                  throw new Error(
                    `Fix summaries for run ${run.id}: ${fixError.message}`,
                  );
                }
      ...
      console.log(
        `   Runs: ${migrationStats.migratedRuns}/${migrationStats.totalRuns}`,
      );
      console.log(
        `   Findings: ${migrationStats.migratedFindings}/${migrationStats.totalFindings}`,
      );
      console.log(
        `   Fix Summaries: ${migrationStats.migratedFixSummaries}/${migrationStats.totalFixSummaries}`,
      );
      console.log(`   Errors: ${migrationStats.errors.length}`);
      console.log(
        `   Duration: ${(migrationStats.duration / 1000).toFixed(2)}s`,
      );
  ...
  async validateSync(options = {}) {
    ...
    const sqliteRuns = await this.sqlite.all(
    ...
    for (const sqliteRun of recentSqliteRuns) {
      ...
      if (!supabaseRuns || supabaseRuns.length === 0) {
        ...
        const fieldsMatch =
          ...
          Math.abs(
            (sqliteRun.quality_score || 0) - (supabaseRun.quality_score || 0),
          ) < 0.01;
    ...
    console.log(
      `   Sample checks: ${validation.sampleChecks.filter((c) => c.status === "consistent").length}/${validation.sampleChecks.length} consistent`,
    );
