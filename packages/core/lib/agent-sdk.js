...
class AgentSDK extends EventEmitter {
  ...
  updateMessageBusReport() {
    if (
      !this.messageBus ||
      typeof this.messageBus.updateAgentSdkReport !== "function"
    ) {
      return;
    }
    ...
    try {
      ...
      callLogger(
        this.logger,
        "warn",
        `[AgentSDK] Failed to push telemetry report: ${error.message}`,
      );
    }
  }
  ...
  startSession({ status = "active" } = {}) {
    ...
    if (identifier) {
      ...
      callLogger(
        this.logger,
        "warn",
        "[AgentSDK] Unable to resolve agent identifier during startSession",
      );
    }
    ...
    this._heartbeatTimer = setInterval(() => {
      try {
        ...
        callLogger(
          this.logger,
          "warn",
          `[AgentSDK] Heartbeat error: ${error.message}`,
        );
      }
    ...
    if (identifier) {
      ...
      if (!assigned) {
        callLogger(
          this.logger,
          "warn",
          `[AgentSDK] Failed to assign task ${effectiveTaskId} for ${identifier}`,
        );
      }
    }
  ...
  completeTask({
    ...
    this.runtimeState = {
      ...
      toolBudget: toolBudget
        ? { ...toolBudget }
        : { limit: null, used: toolCallsUsed, remaining: null },
      evidenceCount,
      verificationStatus,
      toolCallsUsed,
    ...
    if (identifier && resolvedTaskId) {
      ...
      callLogger(
        this.logger,
        "warn",
        "[AgentSDK] Unable to resolve agent identifier during completeTask",
      );
    }
  ...
  report() {
    ...
    const successRate =
      tasksAttempted === 0 ? 1 : tasksSucceeded / tasksAttempted;
    const averageExecutionTime =
      tasksSucceeded === 0
        ? 0
        : totalExecutionTime / Math.max(tasksSucceeded, 1);
