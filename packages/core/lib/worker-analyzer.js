...
class WorkerAnalyzer {
  ...
  logError(context, error) {
    console.error(
      `[Worker ${this.workerId}] ERROR [${context}]:`,
      error?.message || error,
    );
    if (error?.stack) {
  ...
  async analyzeBatch(batch, options = {}) {
    if (!batch || !batch.files || !Array.isArray(batch.files)) {
      this.logError(
        "Invalid batch",
        new Error("Batch must have a files array"),
      );
      this.sendMessage({
        type: "error",
        workerId: this.workerId,
    ...
    try {
      for (const filePath of batch.files) {
        ...
        if (
          ...
          Date.now() - batchStart > options.maxDurationMs
        ) {
        ...
        try {
          ...
          const timeoutPromise = new Promise((_, reject) => {
            timeoutHandle = setTimeout(
              () => reject(new Error("File analysis timeout")),
              perFileTimeout,
            );
            timeoutHandle.unref?.();
          });
  ...
  async analyzeFile(filePath, options = {}) {
    ...
    for (const domain of this.domains) {
      try {
        ...
        console.error(
          `[Worker] Error analyzing ${domain} for ${filePath}:`,
          error.message,
        );
      }
    }
