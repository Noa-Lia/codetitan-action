...
class ClaudeProvider extends AIProvider {
  constructor(config = {}) {
    ...
    const pricing =
      modelPricing[defaultModel] || modelPricing["claude-sonnet-4-6-20260217"];
    ...
    if (this.enabled) {
      try {
        ...
        console.warn(
          "[ClaudeProvider] @anthropic-ai/sdk not installed. Run: npm install @anthropic-ai/sdk",
        );
        this.enabled = false;
      }
    }
  ...
  async analyze(domain, filePath, content, projectRoot, options = {}) {
    if (!this.enabled || !this.client) {
      throw new Error(
        "ClaudeProvider is not available. Check API key and dependencies.",
      );
    }
  ...
  calculateActualCost(usage) {
    ...
    const inputCost =
      (inputTokens - cachedTokens - writeCacheTokens) * this.costPerInputToken;
