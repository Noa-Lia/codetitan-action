...
const INJECTION_RULES = [
  ...
  },
  {
    ...
    severity: 'HIGH',
    impact: 8,
    pattern: /res\.setHeader\s*\([^)]*(?:req\.|request\.|params\.|query\.|body\.|input)[^)]*\)/,
  ...
  },
  {
    ...
    severity: 'HIGH',
    impact: 8,
    pattern: /(?:to|cc|bcc)\s*:\s*(?:req\.|request\.|params\.|query\.|body\.|input|`[^`]*\$\{)/,
...
const MISCONFIG_RULES = [
  ...
  {
    id: 'CLICKJACKING_NO_HEADER',
    severity: 'MEDIUM',
    impact: 6,
    pattern: /(?:const|let|var)\s+(?:app|router|server)\s*=\s*express\s*\(\s*\)/,
    fileGuard: /helmet|frameguard|X-Frame-Options|frame-ancestors/,
    message: 'Express app created without X-Frame-Options or Content-Security-Policy frame-ancestors — clickjacking vulnerability. Use helmet() or set the header manually.',
    skipTest: true,
    skipDoc: true,
  },
  {
    ...
    impact: 6,
    pattern: /(?:const|let|var)\s+(?:app|router|server)\s*=\s*express\s*\(\s*\)/,
    fileGuard: /helmet|hsts|Strict-Transport-Security/,
    message: 'Express app created without Strict-Transport-Security (HSTS) — browsers may allow downgrade to HTTP. Add HSTS with a max-age of at least 1 year via helmet() or manually.',
    skipTest: true,
    skipDoc: true,
  },
  {
  ...
  },
  {
    ...
    severity: 'HIGH',
    impact: 7,
    pattern: /(?:router|app)\.get\s*\([^)]*(?:password|token|secret|apiKey|api_key|ssn|creditCard)[^)]*,/i,
...
const CLIENT_RULES = [
  ...
  },
  {
    ...
    severity: 'HIGH',
    impact: 8,
    pattern: /(?:setTimeout|setInterval)\s*\(\s*(?:`[^`]+`|['"][^'"]+['"]|[a-z_$][\w.]*(?:\[['"\w]+\])?)\s*,/,
...
const INFRA_RULES = [
  ...
  },
  {
    ...
    severity: 'HIGH',
    impact: 8,
    pattern: /^(?:USER\s+root|(?!.*\bUSER\b).*(?:RUN|CMD|ENTRYPOINT))/m,
    message: 'Docker container runs as root — process escape allows full host access. Add a USER instruction with a non-root UID before CMD/ENTRYPOINT.',
    skipTest: true,
    skipDoc: true,
  },
  {
