...
class ToolRouter {
  ...
  async listFiles(input) {
    const startedAt = Date.now();
    const raw = await this.toolBridge.listFiles(
      input.path,
      input.options || {},
    );
  ...
  async searchCode(input) {
    ...
    return createToolResult({
      ...
      metadata: {
        ...
        usage: {
          bytesTouched: Buffer.byteLength(
            JSON.stringify(raw.matches || []),
            "utf8",
          ),
          tokensTouched: 0,
        },
    ...
    if (!raw.success) {
      return createToolResult({
        ...
        error:
          raw.error ||
          raw.stderr ||
          `Command exited with status ${raw.exitCode}`,
        data: {
          command: raw.command,
          args: raw.args || [],
          cwd: raw.cwd,
  ...
  async createWorktree(input, context = null) {
    ...
    if (context && typeof context.registerWorkspace === "function") {
      ...
      if (
        typeof context.registerCleanup === "function" &&
        typeof this.toolBridge.removeWorktree === "function"
      ) {
        context.registerCleanup(
          `remove_worktree:${raw.id || raw.path}`,
          async () => {
            ...
            if (!removal.success) {
              throw new Error(
                removal.error || `Unable to remove workspace ${raw.path}`,
              );
            }
  ...
  async editFile(input) {
    const startedAt = Date.now();
    const raw = await this.toolBridge.edit(
      input.file,
      input.oldString,
      input.newString,
    );
  ...
  async browseWeb(input) {
    ...
    return createToolResult({
      ...
      metadata: {
        ...
        usage: {
          bytesTouched:
            raw.bytesTouched || Buffer.byteLength(raw.text || "", "utf8"),
          tokensTouched: 0,
        },
      },
      startedAt,
