const BUILTIN_ROLE_PROFILES = {
  ...
  fixer: {
    name: "fixer",
    description:
      "Prepares fix candidates and applies bounded edits inside isolated worktrees.",
    allowedTools: [
      "create_worktree",
      "edit_file",
      "promote_worktree",
  ...
  verifier: {
    name: "verifier",
    description:
      "Checks outputs and validation evidence without mutating files.",
    allowedTools: [
      "read_file",
      "list_files",
      "analyze_path",
  ...
  orchestrator: {
    name: "orchestrator",
    description:
      "Coordinates and synthesizes evidence without producing fix artifacts directly.",
    allowedTools: [
      "read_file",
      "list_files",
      "analyze_path",
...
function normalizeRoleName(value) {
  ...
  if (
    ...
    normalized.includes("code intelligence")
  ) {
    return "researcher";
  }
  if (
...
function getRoleProfile(roleName) {
  ...
  const baseProfile =
    BUILTIN_ROLE_PROFILES[resolvedRoleName] || BUILTIN_ROLE_PROFILES.reviewer;
