...
function validateSchema(schema = {}, input = {}) {
  ...
  Object.entries(properties).forEach(([propertyName, propertySchema]) => {
    ...
    if (
      propertySchema.type &&
      !validatePrimitive(propertySchema.type, propertyValue)
    ) {
    ...
    if (
      ...
      !propertySchema.enum.includes(propertyValue)
    ) {
      errors.push(
        `Field ${propertyName} must be one of: ${propertySchema.enum.join(", ")}`,
      );
      return;
    }
    ...
    if (
      ...
      errors.push(
        `Field ${propertyName} must be at least ${propertySchema.minLength} characters`,
      );
    }
    ...
    if (propertySchema.type === "number") {
      if (
        ...
        errors.push(
          `Field ${propertyName} must be at least ${propertySchema.minimum}`,
        );
      }
      if (
        ...
        errors.push(
          `Field ${propertyName} must be at most ${propertySchema.maximum}`,
        );
      }
    }
    ...
    if (propertySchema.type === "array") {
      if (
        ...
        propertyValue.length < propertySchema.minItems
      ) {
        errors.push(
          `Field ${propertyName} must contain at least ${propertySchema.minItems} item(s)`,
        );
      }
      if (
        typeof propertySchema.maxItems === "number" &&
        propertyValue.length > propertySchema.maxItems
      ) {
        errors.push(
          `Field ${propertyName} must contain at most ${propertySchema.maxItems} item(s)`,
        );
      }
      if (propertySchema.items && propertySchema.items.type) {
        propertyValue.forEach((item, index) => {
          if (!validatePrimitive(propertySchema.items.type, item)) {
            errors.push(
              `Field ${propertyName}[${index}] must be a ${propertySchema.items.type}`,
            );
            return;
          }
          ...
          if (
            ...
            !propertySchema.items.enum.includes(item)
          ) {
            errors.push(
              `Field ${propertyName}[${index}] must be one of: ${propertySchema.items.enum.join(", ")}`,
            );
          }
        });
      }
...
const DEFAULT_TOOL_DEFINITIONS = [
  ...
  },
  {
    name: "git_diff",
    description:
      "Capture a git diff for the working tree, staged changes, or a revision range.",
    inputSchema: {
      type: "object",
      additionalProperties: false,
      properties: {
  ...
  },
  {
    name: "fetch_history",
    description:
      "Read persisted local analysis history for the active project or a specific run.",
    inputSchema: {
      type: "object",
      additionalProperties: false,
      properties: {
  ...
  },
  {
    ...
    description:
      "Create an isolated worktree or copy-based workspace for fixer-style execution.",
    inputSchema: {
      type: "object",
      additionalProperties: false,
      properties: {
  ...
  },
  {
    name: "edit_file",
    description:
      "Edit a file by replacing an explicit source string with a replacement string.",
    inputSchema: {
      type: "object",
  ...
  },
  {
    ...
    description:
      "Promote validated files from the active isolated worktree back into the repository.",
    inputSchema: {
      type: "object",
      required: ["files"],
      additionalProperties: false,
  ...
  },
  {
    name: "browse_web",
    description:
      "Use the managed browser MCP bridge to inspect a web page without raw shell access.",
    inputSchema: {
      type: "object",
      required: ["url"],
      additionalProperties: false,
      properties: {
        ...
        action: {
          type: "string",
          enum: ["read", "screenshot", "click", "type"],
        },
  ...
  },
  {
    ...
    description:
      "Post inline review comments for findings to a GitHub pull request using structured arguments.",
    inputSchema: {
      type: "object",
  ...
  },
  {
    ...
    description:
      "Record a proposed fix candidate without mutating source files.",
    inputSchema: {
      type: "object",
      required: ["candidates"],
      additionalProperties: false,
