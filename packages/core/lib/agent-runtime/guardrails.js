...
class Guardrails {
  ...
  assertStepBudget(context) {
    if (context.stepsTaken >= this.options.maxSteps) {
      throw new Error(
        `Agent runtime step budget exceeded (${this.options.maxSteps})`,
      );
    }

    if (
      context.promptBudget &&
      typeof context.promptBudget.toolCap === "number" &&
      context.stepsTaken >= context.promptBudget.toolCap
    ) {
      throw new Error(
        `Agent runtime tool budget exceeded (${context.promptBudget.toolCap})`,
      );
    }
  }
  ...
  assertTool(definition, input = {}, context = null) {
    ...
    const roleProfile =
      context && context.roleProfile ? context.roleProfile : null;

    if (
      roleProfile &&
      Array.isArray(roleProfile.allowedTools) &&
      !roleProfile.allowedTools.includes(definition.name)
    ) {
      throw new Error(
        `Role ${roleProfile.name} cannot use tool ${definition.name}`,
      );
    }

    if (
      definition.mutating &&
      !this.options.allowMutatingTools &&
      !(roleProfile && roleProfile.allowMutatingTools === true)
    ) {
      throw new Error(
        `Mutating tool blocked by guardrails: ${definition.name}`,
      );
    }
    ...
    if (
      definition.mutating &&
      roleProfile &&
      roleProfile.allowMutatingTools === false
    ) {
      throw new Error(
        `Role ${roleProfile.name} cannot mutate files with tool ${definition.name}`,
      );
    }
    ...
    if (
      definition.mutating &&
      roleProfile &&
      roleProfile.requiresWorktreeForMutations
    ) {
  ...
  assertMutationTargetsInWorkspace(input = {}, context = null) {
    ...
    if (!workspace || !workspace.path) {
      throw new Error(
        "Mutating fixer tools require an active isolated worktree",
      );
    }
  ...
  assertProviderBudget(context, usage = {}) {
    ...
    const projectedCost =
      (context.providerUsage?.totalCostUsd || 0) +
      (usage.costUSD || usage.costUsd || 0);

    if (tokenCap > 0 && projectedTokens > tokenCap) {
      throw new Error(
        `Agent runtime token budget exceeded (${projectedTokens} > ${tokenCap})`,
      );
    }

    if (usdCap > 0 && projectedCost > usdCap) {
      throw new Error(
        `Agent runtime USD budget exceeded (${projectedCost} > ${usdCap})`,
      );
    }
  }
}
