...
class IncidentWebhook {
  constructor(feedbackLoop, options = {}) {
    ...
    this.options = {
      ...
      sharedSecretHeader:
        options.sharedSecretHeader || "x-codetitan-webhook-secret",
      ...
      try {
        ...
        if (!incident.error) {
          return res.status(400).json({
            error: "Cannot parse incident: missing error/message field",
          });
        }
        ...
        res.json({
          ...
          newRuleCandidate: result.newRuleCandidate
            ? result.newRuleCandidate.ruleId
            : null,
        });
      } catch (err) {
      ...
      try {
        ...
        await this.feedbackLoop.recordOutcome(
          `deploy-${version || "unknown"}`,
          {
            ...
            errorMessage:
              status === "failed"
                ? `Deployment ${version} failed in ${environment}`
                : null,
          },
        );
        res.json({ ok: true });
  ...
  parseIncident(body, source = "generic") {
    ...
    switch (source) {
      ...
      case "pagerduty": {
        ...
        severity = this._mapPagerDutySeverity(
          incident.urgency || incident.priority?.name,
        );
        break;
      }
  ...
  async _authorizeWebhookRequest(req) {
    ...
    const sharedSecretValid =
      typeof sharedSecret === "string" && sharedSecret === secret;
    ...
    if (signatureValid) {
      ...
      if (
        previousSeenAt &&
        now - previousSeenAt < this.options.replayWindowMs
      ) {
    ...
    if (!context.projectId || !context.tenantId) {
      return {
        ok: false,
        status: 400,
        error: "project_id and tenant_id are required",
      };
    }
    ...
    if (typeof this.options.verifyProjectBinding === "function") {
      ...
      if (!isValidBinding) {
        return {
          ok: false,
          status: 403,
          error: "Project binding is invalid for this tenant",
        };
      }
    }
  ...
  _verifySignature(payload, signature, timestamp) {
    ...
    const normalizedSignature = signature.startsWith("v1=")
      ? signature.slice(3)
      : signature;
