...
class GitWorktreeManager {
  ...
  createWorktree(options = {}) {
    ...
    const explicitTargetPath = options.targetPath
      ? this.resolvePathInsideRepo(
          path.relative(
            this.repoPath,
            path.resolve(this.repoPath, options.targetPath),
          ),
        )
      : null;
    const handle = {
    ...
    if (this.isGitRepository()) {
      ...
      throw new Error(
        "Repository is not a git repository and fallback copy mode is disabled",
      );
    }
  ...
  normalizeHandle(handleOrPath) {
    ...
    if (typeof handleOrPath === "string") {
      ...
      const existing = Array.from(this.handles.values()).find(
        (handle) => handle.path === path.resolve(handleOrPath),
      );
      if (existing) {
        return existing;
      }
  ...
  promoteFiles(handleOrPath, files = []) {
    ...
    const relativeFiles = files.map((filePath) =>
      this.toRepoRelativePath(filePath),
    );
    ...
    relativeFiles.forEach((relativePath) => {
      ...
      if (!fs.existsSync(source) || !fs.statSync(source).isFile()) {
        throw new Error(
          `Worktree file not found for promotion: ${relativePath}`,
        );
      }
  ...
  captureDiff(handleOrPath, options = {}) {
    ...
    const args = [
      "diff",
      "--no-ext-diff",
      `--unified=${Math.max(0, Number(options.unified) || 3)}`,
    ];
    const files = Array.isArray(options.files)
      ? options.files.map((filePath) => this.toRepoRelativePath(filePath))
      : [];
