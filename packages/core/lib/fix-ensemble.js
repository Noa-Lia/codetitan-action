...
class GitHelper {
  ...
  createWorktree(worktreePath, options = {}) {
    ...
    const relativeBaseDir = path.relative(
      this.repoPath,
...
class DryRunSimulator {
  ...
  detectIssues(original, modified, fix) {
    ...
    const dangerPatterns = [
      ...
      {
        pattern: /Function\(/g,
        message: "Fix introduces Function constructor",
      },
      {
        pattern: /innerHTML\s*=/g,
        message: "Fix introduces innerHTML assignment",
      },
    ];
  ...
  getSummary() {
    return {
      ...
      withIssues: this.changes.filter((c) => c.potentialIssues.length > 0)
        .length,
...
class FixEnsembleVerifier {
  ...
  async verifyFix(fix, context = {}) {
    ...
    let action;
    if (
      consensus >= CONSENSUS_THRESHOLDS.AUTO_APPLY &&
      consensus >= riskConfig.requiredConsensus
    ) {
      action = "AUTO_APPLY";
  ...
  async applyFix(fix, verification, context = {}) {
    ...
    try {
      if (!useDirectMode) {
        const resolvedFiles = this.resolveFixFiles(fix, context).map((file) =>
          this.git.toRepoRelativePath(file),
        );
        ...
        for (const relativeFile of resolvedFiles) {
          ...
          if (!applied) {
            throw new Error(
              `Unable to apply fix in isolated worktree: ${relativeFile}`,
            );
          }
        }

        const diffSummary = this.git.captureDiff(worktreeHandle, {
          files: resolvedFiles,
        });
    ...
    return {
      ...
      avgConsensus:
        verifications.length > 0
          ? verifications.reduce((sum, v) => sum + v.consensus, 0) /
            verifications.length
          : 0,
      byAction: {
        AUTO_APPLY: verifications.filter((v) => v.action === "AUTO_APPLY")
          .length,
      ...
      byRisk: {
        CRITICAL: verifications.filter((v) => v.riskLevel === "CRITICAL")
          .length,
