...
function vulnerabilityToFinding(vuln, pkg, manifestPath) {
  ...
  if (vuln.severity && vuln.severity.length > 0) {
    const cvss =
      vuln.severity.find((s) => s.type === "CVSS_V3") || vuln.severity[0];
...
class DependencyScanner {
  ...
  async scan(projectPath, options = {}) {
    ...
    log(
      `   Found ${results.findings.length} vulnerabilities in ${vulnerablePackages.size} packages`,
    );
    ...
    if (options.reachability !== false && results.findings.length > 0) {
      ...
      log(
        `   Running reachability analysis across ${sourceFiles.length} source file(s)...`,
      );
      results.findings = await reachability.analyzeReachability(
        results.findings,
        sourceFiles,
      );
      results.summary.reachableVulnerabilities = results.findings.filter(
        (f) => f.reachable === true,
      ).length;
      results.summary.unreachableVulnerabilities = results.findings.filter(
        (f) => f.reachable === false,
      ).length;
      log(
        `   Reachable: ${results.summary.reachableVulnerabilities}, Unreachable/unconfirmed: ${results.summary.unreachableVulnerabilities}`,
      );
    }
  ...
  async scanTransitive(projectPath, _options = {}) {
    ...
    if (allDeps.length === 0) {
      return {
        findings: [],
        summary: { total: 0, direct: 0, transitive: 0, bySeverity: {} },
      };
    }
      ...
      try {
        ...
        for (let j = 0; j < vulns.length; j++) {
          ...
          for (const vuln of vulnList) {
            const sev = getSeverityFromCVSS(
              vuln.database_specific?.severity || 0,
            );
            findings.push({
