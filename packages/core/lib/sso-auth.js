...
class SSOSession {
  constructor(options) {
    ...
    this.expiresAt =
      options.expiresAt ?? new Date(Date.now() + 8 * 60 * 60 * 1000); // 8 hours
...
const RBAC_ROLES = {
  ...
  OWNER: {
    name: "Owner",
    permissions: ["project:*", "team:*", "billing:*", "settings:*"],
  },
  MANAGER: {
    name: "Manager",
    permissions: [
  ...
  DEVELOPER: {
    name: "Developer",
    permissions: ["project:read", "analysis:run", "analysis:read", "fix:apply"],
  },
  VIEWER: {
    name: "Viewer",
    permissions: ["project:read", "analysis:read"],
  },
};
