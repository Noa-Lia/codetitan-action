...
class EnhancedTaintTracker {
  ...
  markTainted(varName, source, trustLevel = 0, location = null) {
    ...
    this.taintedVars.set(varName, {
      source,
      trustLevel: existing
        ? Math.min(existing.trustLevel, trustLevel)
        : trustLevel,
      location,
      timestamp: Date.now(),
      propagationPath: existing
        ? [...(existing.propagationPath || []), source]
        : [source],
    });
...
function getRemediation(sinkType) {
  const remediations = {
    CODE_EXECUTION:
      "Never use dynamic code execution with user input. Use a sandboxed environment like vm2.",
    COMMAND_INJECTION:
      "Use parameterized commands or shell-escape libraries. Avoid shell: true.",
    SQL_INJECTION:
      "Use parameterized queries or an ORM. Never concatenate user input into SQL.",
    NOSQL_INJECTION:
      "Validate object keys and reject $where operators from user input.",
    XSS: "Use textContent instead of innerHTML, or sanitize with DOMPurify.",
    PATH_TRAVERSAL:
      "Use path.basename() and validate against a whitelist of allowed paths.",
    OPEN_REDIRECT:
      "Use a whitelist of allowed redirect URLs or validate the URL domain.",
    SSRF: "Validate and whitelist allowed URLs/hosts. Block internal IP ranges.",
    DESERIALIZATION:
      "Validate JSON schema before parsing. Use safe deserialization libraries.",
...
function analyzeDataFlow(code, filePath, options = {}) {
    ...
    CallExpression(nodePath) {
      ...
      Object.entries(SINKS).forEach(([sinkType, sink]) => {
        ...
        if (
          ...
          sink.functions?.includes(callee.name)
        ) {
          isSinkMatch = true;
        ...
        if (isSinkMatch) {
          ...
          args.forEach((arg, index) => {
            ...
            if (arg.type === "Identifier" && tracker.isTainted(arg.name)) {
              ...
              findings.push(
                createFinding(node, sinkType, sink, taintInfo, code, context),
              );
            }
  ...
  return {
    ...
    callGraph: Object.fromEntries(
      Array.from(tracker.callGraph.entries()).map(([k, v]) => [
        k,
        Array.from(v),
      ]),
    ),
