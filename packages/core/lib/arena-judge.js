...
class ArenaJudge {
  ...
  async evaluate(
    ...
    result.newSecurityIssues = await this.runSecurityCheck(
      fixedCode,
      filePath,
      beforeFindings,
    );
  ...
  async runSecurityCheck(fixedCode, filePath, beforeFindings) {
    try {
      const { analyzeDomain } = require("./domain-analyzers");
      const afterIssues = analyzeDomain(
        "security-god",
        filePath,
        fixedCode,
        this.projectRoot,
      ...
      const beforeCount =
        (beforeFindings?.bySeverity?.CRITICAL || 0) +
        (beforeFindings?.bySeverity?.HIGH || 0);
      const afterCount = serious.length;

      if (afterCount > beforeCount) {
        return serious
          .slice(0, afterCount - beforeCount)
          .map((i) => i.message || i.id || "unknown");
      }
      return [];
    } catch {
      return [];
  ...
  calculateScore(evaluation) {
    ...
    const testScore =
      evaluation.testsPass || evaluation.testResults?.noTests ? 1.0 : 0.0;
    ...
    const confidenceScore =
      evaluation.confidenceScore != null ? evaluation.confidenceScore : 0.5;
