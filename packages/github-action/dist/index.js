...
async function ensureBundledCoreRuntime(repoRoot) {
    ...
    if (fs.existsSync(installMarker) &&
        process.env.CODETITAN_ACTION_FORCE_CORE_REFRESH !== "1") {
    ...
    ensureRuntimeManifest(runtimeRoot);
    let packStdout = "";
    await action_kit_1.exec.exec("npm", [
        "pack",
...
function getGitDiffUtils(coreApi) {
    ...
    if (!utils ||
        typeof utils.listChangedFiles !== "function" ||
        typeof utils.createChangedFilesWorkspace !== "function") {
...
function filterProvidedChangedFiles(coreApi, rootPath, changedFiles) {
    ...
    return (changedFiles || [])
        ...
        if (normalizedFile !== normalizedRoot &&
            !normalizedFile.startsWith(normalizedRootWithSep)) {
            return false;
        }
...
function classifyFinding(category) {
    ...
    if (normalized === "COMMAND_EXEC" ||
        ...
        normalized === "CROSS_FILE_TAINT_COMMAND_INJECTION" ||
        normalized === "EVAL_USAGE" ||
        normalized === "TAINT_EVAL" ||
        normalized === "CROSS_FILE_TAINT_EVAL" ||
    ...
    if (normalized === "SQL_INJECTION" ||
        ...
        normalized === "CROSS_FILE_TAINT_SQL_INJECTION" ||
        ...
        normalized === "CROSS_FILE_TAINT_NOSQL_INJECTION" ||
    ...
    if (normalized === "XSS" ||
        ...
        normalized === "CROSS_FILE_TAINT_XSS" ||
        ...
        normalized === "CROSS_FILE_TAINT_TEMPLATE_INJECTION") {
    ...
    if (normalized === "PROTOTYPE_POLLUTION" ||
        ...
        normalized === "CROSS_FILE_TAINT_OPEN_REDIRECT" ||
        ...
        normalized === "CROSS_FILE_TAINT_INSECURE_DESERIALIZATION" ||
...
async function runCoreAnalysisWith(coreApi, projectPath, level, options = {}) {
    ...
    let changedOnly = options.changedOnly === true;
    const changedOnlyExplicit = options.changedOnlyExplicit === true;
    ...
    if (changedOnly) {
        ...
        const providedChangedFiles = Array.isArray(options.changedFiles)
            ...
            : null;
        // Defense-in-depth: wrap the local-git fallback in a narrow catch that
        // matches ONLY the "changed-only requires a git repository" error from
        // packages/core/lib/git-diff-utils.js:68-71. Other diff errors (shallow
        // clones, missing diff base, permission failures) still throw through
        // as hard failures. Per Codex critique #236 sections 2 + 6.
        let diff;
        if (providedChangedFiles) {
            ...
            try {
                diff = gitDiffUtils.listChangedFiles(projectPath, {
                    baseRef: options.diffBase,
                });
            }
            catch (error) {
                const message = error instanceof Error ? error.message : String(error);
                const isMissingRepo = /changed-only requires a git repository/.test(message);
                if (!isMissingRepo)
                    throw error;
                // PR-default downgrade: only when user did NOT explicitly request
                // changed-only. Explicit changed-only=true preserves user intent
                // and throws through.
                if (changedOnlyExplicit)
                    throw error;
                try {
                    coreApi?.warning?.(`[codetitan-action] changed-only requested by PR default but ` +
                        `target is not inside a git repository — downgrading to full ` +
                        `analysis. Set 'changed-only: false' in workflow to silence ` +
                        `this warning, or run with a git checkout to enable scoped diff.`);
                }
                catch (_) {
                    /* coreApi.warning is optional in some test contexts */
                }
                changedOnly = false;
                diff = { changedFiles: [], diffSpec: "" }; // unused — fall through to full analysis
            }
        }
        // If we fell through to full analysis on downgrade, skip changed-scope setup.
        if (!changedOnly) {
            // changedScope stays undefined → report summary shows changedOnly: false.
            // analysisRoot stays as rootPath → full analysis runs below.
        }
        else {
            ...
            changedScope = {
                ...
                diffSource: providedChangedFiles
                    ? options.diffSource || "github-api"
                    : "git-diff",
            };
            if (isFileTarget) {
        ...
        } // end inner else (changedOnly still true after defense-in-depth catch)
    }
    ...
    const orchestrator = new CodeTitanOrchestration({
        ...
        aiGenerated: true,
        fpFilter: false,
        // Constructor option for Python cross-file taint kill-switch. Merged
        // with per-call options at the core orchestration layer; read at the
        // orchestrator wire site. Env var CODETITAN_DISABLE_CROSS_FILE=1 is
        // honored independently. Added 2026-05-18 audit-resolution patch.
        disableCrossFile: options.disableCrossFile === true,
        // Constructor option for JS/TS cross-file taint kill-switch. Distinct
        // from `disableCrossFile` so Python and JS passes can be toggled
        // independently. Env var `CODETITAN_DISABLE_CROSS_FILE_JS=1` is
        // honored at the orchestrator wire site independently. Added
        // 2026-05-18 js-wire-in.
        disableCrossFileJs: options.disableCrossFileJs === true,
    });
    let coreReport;
    try {
    ...
    const findings = (coreReport.findings || []).map((issue) => {
        ...
        const absoluteFile = rawPath
            ? path.isAbsolute(rawPath)
                ? rawPath
                : path.resolve(analysisRoot, rawPath)
            : "";
    ...
    const filteredFindings = targetFile
        ? findings.filter((finding) => {
            ...
            return (normalizePath(path.resolve(rootPath, finding.file_path)) ===
                targetFile);
        })
        : findings;
    ...
    const report = {
        ...
        files_analyzed: targetFile
            ? 1
            : coreReport.summary?.totalFiles ||
                new Set(filteredFindings.map((finding) => finding.file_path)).size,
        files_skipped: 0,
        ...
        projectName: isFileTarget
            ? path.basename(projectPath)
            : path.basename(path.resolve(projectPath)),
        learnedProfile: coreReport.learnedProfile || null,
        prRiskScore: coreReport.prRiskScore ||
            coreReport.metrics?.prRiskScore ||
            coreReport.summary?.prRiskScore ||
            null,
    };
    report.summary = {
        ...(report.summary || {}),
    ...
    if (changedOnly) {
        report.files_analyzed = isFileTarget
            ? 1
            : changedScope?.changedFiles.length || 0;
    }
    return report;
}
...
async function runActionAnalysis(projectPath, level, options = {}) {
    ...
    if (bundledRepoRoot &&
        !forcePackRuntime &&
        hasBundledCoreWorkspace(bundledRepoRoot)) {
        ...
        runs: [
            {
                tool: {
                    driver: {
                        ...
                        rules: [
                            ...new Set(findings.map((finding) => finding.category || "unknown")),
                        ].map((id) => ({
                            id,
...
function buildRepoSignalsSection(report) {
    ...
    if (aiAttrib &&
        typeof aiAttrib.coverage === "number" &&
        aiAttrib.coverage > 0) {
...
async function upsertPullRequestComment(token, markdown, prContext) {
    // Backward-compat: when prContext is not supplied, fall back to reading
    // global context directly (preserves existing dist/index.js behavior for
    // external consumers).
    const pr = prContext === undefined
        ? action_kit_1.github.context.payload.pull_request
            ? { number: action_kit_1.github.context.payload.pull_request.number }
            : null
        : prContext;
    if (!pr) {
        return;
    ...
    const { owner, repo } = action_kit_1.github.context.repo;
...
async function fetchPullRequestChangedFiles(token, prContext) {
    // Backward-compat: when prContext is not supplied, fall back to reading
...
async function run() {
    try {
        ...
        const changedOnlyInput = (action_kit_1.core.getInput("changed-only") || "")
            .trim()
            .toLowerCase();
        // Normalize PR context ONCE per #227 commit 2/2. Helpers receive this
        // normalized object instead of re-reading github.context.payload across
        // three sites (run(), upsertPullRequestComment, fetchPullRequestChangedFiles).
        // Codex critique #236 section 3 — repo intentionally not normalized here.
        const prPayload = action_kit_1.github.context.payload.pull_request;
        const prContext = prPayload
            ? { number: prPayload.number }
            : null;
        const isPullRequestEvent = prContext !== null;
        // `changedOnlyExplicit` distinguishes "user opted in via input=true"
        // from "PR-default inferred from pull_request event". The action-runtime
        // uses this to decide whether a missing-git scenario should hard-fail
        // (explicit intent honored) or silently downgrade to full analysis
        // (PR default, no usable diff source). See #227 + docs/plans/
        // 2026-05-17-codex-227-plan-critique.md.
        const changedOnlyExplicit = changedOnlyInput === "true";
        const changedOnly = changedOnlyExplicit || (changedOnlyInput === "" && isPullRequestEvent);
        const diffBase = action_kit_1.core.getInput("diff-base") ||
            action_kit_1.github.context.payload.pull_request?.base?.sha ||
            "";
        ...
        // 2026-05-18 audit-resolution patch: operator kill-switch for the
        // Python cross-file taint pass. Default 'false' preserves existing
        // behavior; setting 'true' skips the cross-file pass at the
        // orchestrator wire site.
        const disableCrossFile = action_kit_1.core.getInput("disable-cross-file") === "true";
        // 2026-05-18 js-wire-in: operator kill-switch for the JS/TS
        // cross-file taint pass. Distinct from `disable-cross-file` (Python)
        // so the two passes can be toggled independently. Forwarded to
        // `runCoreAnalysisWith` which surfaces it to the orchestrator
        // constructor. Env var `CODETITAN_DISABLE_CROSS_FILE_JS=1` is honored
        // independently at the wire site.
        const disableCrossFileJs = action_kit_1.core.getInput("disable-cross-file-js") === "true";
        ...
        if (changedOnly && isPullRequestEvent && githubToken) {
            const prFiles = await fetchPullRequestChangedFiles(githubToken, prContext);
        ...
        const passed = !hasBlockingFindings(findings, failOnSeverity) &&
            (report.prRiskScore?.score || 0) < riskThreshold;
        ...
        if (commentOnPR && githubToken) {
            await upsertPullRequestComment(githubToken, markdownSummary, prContext);
        }
